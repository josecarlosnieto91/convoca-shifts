...
function cst_register_rest_routes() {
    register_rest_route( 'centro/v1', '/turnos', array(
        'methods'  => 'GET',
        'callback' => 'cst_rest_get_turnos',
        'permission_callback' => '__return_true', // Publicly readable
    ) );

    register_rest_route( 'centro/v1', '/turnos/(?P<id>\d+)/apuntarse', array(
        'methods'  => 'POST',
    ...
    register_rest_route( 'centro/v1', '/turnos/(?P<id>\d+)/desapuntarse', array(
        'methods'  => 'POST',
    ...
    register_rest_route( 'centro/v1', '/turnos/apuntarse-proximo', array(
        'methods'  => 'POST',
    ...
    register_rest_route( 'centro/v1', '/turnos/proximo-libre', array(
        'methods'  => 'GET',
        'callback' => 'cst_rest_get_proximo_libre',
        'permission_callback' => '__return_true', // Public
    ) );

    register_rest_route( 'centro/v1', '/turnos/crear', array(
        'methods'  => 'POST',
...
function cst_rest_apuntarse_turno( WP_REST_Request $request ) {
    ...
    $current_responsable = $wpdb->get_var( $wpdb->prepare(
    ...
    $hora_fin = get_post_meta( $post_id, '_hora_fin', true );
    $user_shift_lock = $wpdb->get_results( $wpdb->prepare(
        "SELECT p.ID FROM {$wpdb->posts} p
         ...
         AND p.ID != %d
         FOR UPDATE",
        $user_id, $fecha_inicio_turno, $hora_fin, $post_id
    ) );
...
function cst_rest_apuntarse_proximo( WP_REST_Request $request ) {
    ...
    $turno_id = $wpdb->get_var( $wpdb->prepare(
        "SELECT p.ID FROM {$wpdb->posts} p
    ...
    if ( $conflict_id ) {
        $wpdb->query( 'ROLLBACK' );
        if ( function_exists( 'cst_log_activity' ) ) {
            cst_log_activity( $user_id, $turno_id, 'conflicto_horario_detectado', array('automatico' => true, 'conflict_id' => $conflict_id) );
        }
...
function cst_rest_crear_turno( WP_REST_Request $request ) {
    ...
    $post_id = cst_insert_turno( array(
        'date'           => $date,
        'h_start'        => $h_start,
        'h_end'          => $h_end,
        'estado'         => $estado,
