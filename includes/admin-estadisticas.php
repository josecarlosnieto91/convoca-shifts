                    ...
                    <?php foreach ( $logs as $log ) : 
                        ...
                        if ( !$l_turno_title && $log->data ) {
                            ...
                            if ( isset($data['voluntario_id']) ) {
                                ...
                                if ($v) $l_turno_title = __('Voluntario: ', 'convoca-shifts') . $v->display_name;
                            }
                        }
                        if ( !$l_turno_title ) $l_turno_title = $log->turno_id ? 'Turno ID: ' . $log->turno_id : '---';
                        ?>
                        <tr>
        ...
        <?php if ($total_pages > 1) : ?>
            <div class="tablenav" style="margin-top: 10px;">
                <div class="tablenav-pages">
                    <span class="displaying-num"><?php echo sprintf(__('%d elementos', 'convoca-shifts'), $total_logs); ?></span>
...
function cst_exportar_stats_csv_handler() {
    if ( isset( $_POST['cst_action'] ) && $_POST['cst_action'] === 'exportar_stats_csv' && check_admin_referer( 'cst_exportar_stats_action' ) ) {
        if ( ! current_user_can( 'cst_view_stats' ) ) return;
