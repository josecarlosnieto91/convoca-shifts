...
function cst_register_scripts() {
    ...
    wp_localize_script( 'cst-calendario', 'cstData', array(
...
function cst_shortcode_resumen_turnos($atts = []) {
    ...
    <div class="cst-resumen-wrapper">
        ...
        $gaps = get_posts(array(
            'post_type'      => 'centro_turno',
            'posts_per_page' => -1,
            'post_status'    => 'publish',
            'date_query'     => array(
                array( 'after' => $start_of_week, 'before' => $end_of_week, 'inclusive' => true ),
            ),
            'meta_query' => array(
                'relation' => 'AND',
                array( 'key' => '_id_responsable', 'value' => 0, 'compare' => '=' ),
                array( 'key' => '_estado', 'value' => 'abierto_disponible', 'compare' => '=' )
            ),
            'orderby' => 'date',
            'order'   => 'ASC'
        ));
...
function cst_shortcode_proximos_turnos( $atts ) {
    ...
    $args = array(
        ...
        'meta_query' => array(
            array( 'key' => '_estado', 'value' => 'cerrado', 'compare' => '!=' )
        )
    );
