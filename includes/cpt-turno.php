...
function convoca_shifts_sync_turno_on_save( $post_id, $post, $update ) {
	...
	if ( $post->post_type !== 'centro_turno' ) {
		return;
	}
...
function convoca_shifts_register_cpt_centro_turno() {
	...
	add_action( 'convoca_shifts_actividad_add_form_fields', 'Convoca\Shifts\convoca_shifts_actividad_add_meta_fields', 10, 2 );
	add_action( 'convoca_shifts_actividad_edit_form_fields', 'Convoca\Shifts\convoca_shifts_actividad_edit_meta_fields', 10, 2 );
	add_action( 'created_convoca_shifts_actividad', 'Convoca\Shifts\convoca_shifts_save_actividad_meta', 10, 2 );
	add_action( 'edited_convoca_shifts_actividad', 'Convoca\Shifts\convoca_shifts_save_actividad_meta', 10, 2 );
...
add_action( 'init', 'Convoca\Shifts\convoca_shifts_register_cpt_centro_turno' );

// Add metabox for UI configuration of Necesita Apoyo.
add_action( 'add_meta_boxes', 'Convoca\Shifts\convoca_shifts_add_turno_metaboxes' );
	...
	<p>
		...
		<select name="convoca_shifts_id_responsable" id="convoca_shifts_id_responsable" style="width:100%; margin-top:5px;">
			...
			foreach ( $voluntarios as $v ) :
				...
				?>
				<option value="<?php echo esc_attr( $v->ID ); ?>" <?php selected( $id_responsable, $v->ID ); ?>>
	...
	<div id="convoca_shifts_ocupado_fields" style="<?php echo ( $estado === 'abierto_ocupado' ) ? '' : 'display:none;'; ?> background:#f0f7ff; padding:10px; border-radius:4px; margin-top:5px;">
		...
		<select id="convoca_shifts_monitor_select" name="convoca_shifts_monitor_user" style="width:100%;">
			...
			foreach ( $monitor_users as $mu ) :
				...
				?>
				<option value="<?php echo esc_attr( $mu->ID ); ?>" <?php echo esc_attr( $sel ); ?>>
...
add_action( 'save_post_centro_turno', 'Convoca\Shifts\convoca_shifts_save_turno_meta' );
function convoca_shifts_save_turno_meta( $post_id ) {
	if ( ! isset( $_POST['convoca_shifts_turno_meta_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['convoca_shifts_turno_meta_nonce'] ), 'convoca_shifts_turno_meta_action' ) ) {
		return;
	}
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	...
	if ( isset( $_POST['convoca_shifts_estado_real'] ) ) {
		update_post_meta( $post_id, '_estado_real', sanitize_text_field( wp_unslash( $_POST['convoca_shifts_estado_real'] ) ) );
	}

	if ( isset( $_POST['convoca_shifts_estado'] ) ) {
		update_post_meta( $post_id, '_estado', sanitize_text_field( wp_unslash( $_POST['convoca_shifts_estado'] ) ) );
	}
...
add_filter( 'manage_centro_turno_posts_columns', 'Convoca\Shifts\convoca_shifts_set_custom_edit_centro_turno_columns' );
...
add_action( 'manage_centro_turno_posts_custom_column', 'Convoca\Shifts\convoca_shifts_custom_centro_turno_column', 10, 2 );
	...
	switch ( $column ) {
		case 'responsable':
			...
			if ( $id ) {
				...
				echo '<span style="color:#94a3b8;">' . esc_html__( 'Sin asignar', 'convoca-shifts' ) . '</span>';
			}
			break;
		case 'estado_real':
			...
			echo '<div style="margin-bottom:8px;">' . wp_kses_post( $badge_html ) . '</div>';
			...
			if ( $id_responsable > 0 ) {
				...
				) . '" style="color:var(--wp--preset--color--verde, #166534); text-decoration:none;">[Marcar OK]</a> | ';
				echo '<a href="' . esc_url(
					add_query_arg(
						array(
							'status'   => 'no_asistio',
				...
				) . '" style="color:var(--wp--preset--color--granate, #991b1b); text-decoration:none;">[No asistió]</a>';
...
add_action( 'admin_head', 'Convoca\Shifts\convoca_shifts_admin_list_styles' );
...
add_action( 'admin_init', 'Convoca\Shifts\convoca_shifts_handle_admin_attendance_action' );
function convoca_shifts_handle_admin_attendance_action() {
	...
	if ( isset( $get_data['convoca_shifts_action'] ) && $get_data['convoca_shifts_action'] === 'mark_attendance' && isset( $get_data['post'] ) ) {
		...
		if ( $status === 'realizado' && $id_responsable === 0 ) {
			...
			wp_die( esc_html__( 'No se puede marcar como realizado un turno sin responsable asignado.', 'convoca-shifts' ) );
		}
...
add_action( 'quick_edit_custom_box', 'Convoca\Shifts\convoca_shifts_display_quick_edit_turno', 10, 2 );
		...
		<label>
			...
			<select name="convoca_shifts_id_responsable" class="convoca-shifts-quick-responsable">
				...
				foreach ( $voluntarios as $v ) :
					$nombre = ! empty( $v->first_name ) ? $v->first_name : $v->display_name;
					?>
					<option value="<?php echo esc_attr( $v->ID ); ?>"><?php echo esc_html( $nombre . ' (@' . $v->user_login . ')' ); ?></option>
				<?php endforeach; ?>
			</select>
		</label>
		<label>
...
add_action( 'admin_footer', 'Convoca\Shifts\convoca_shifts_quick_edit_javascript' );
...
add_action( 'save_post_centro_turno', 'Convoca\Shifts\convoca_shifts_save_turno_quick_edit', 10, 2 );
function convoca_shifts_save_turno_quick_edit( $post_id, $post ) {
	...
	if ( isset( $_POST['convoca_shifts_estado_real'] ) ) {
		$new_status = sanitize_text_field( wp_unslash( $_POST['convoca_shifts_estado_real'] ) );
...
// --- Exportador CSV ---.
add_action( 'admin_init', 'Convoca\Shifts\convoca_shifts_exportar_csv_turnos' );
...
add_action( 'admin_menu', 'Convoca\Shifts\convoca_shifts_add_generar_turnos_menu', 2 );
...
function convoca_shifts_crear_semana_tipo( $fecha_inicio, $dias ) {
	...
	try {
		...
		if ( ! $start_timestamp ) {
			return new \WP_Error( 'invalid_date', 'Fecha de inicio inválida.' );
		}

		$count = 0;

		foreach ( $dias as $offset => $conf ) {
			if ( ! isset( $conf['activo'] ) || $conf['activo'] != '1' ) {
				continue;
			}
...
function convoca_shifts_insert_turno( $args ) {
	...
	if ( $a['h_end'] > $limit_close ) {
		$a['h_end'] = $limit_close;
	}
	if ( $a['h_start'] >= $a['h_end'] ) {
		$a['h_start'] = $limit_open;
		$a['h_end']   = $limit_close; }
