...
class CST_Admin_Status
{
    ...
    public static function check_pages(): array
    {
        ...
        if ($page && $page->post_status === 'publish') {
            ...
            return self::result('page_calendario', 'Página Calendario',
        ...
        return self::result('page_calendario', 'Página Calendario', 'No encontrada',
            self::SEVERITY_ERROR, 'Crea una página con el shortcode [calendario_centro]');
    }

    public static function check_tables(): array
    {
        global $wpdb;
        $tables = ["{$wpdb->prefix}biodevas_logs", "{$wpdb->prefix}biodevas_locks"];
        $missing = [];
        foreach ($tables as $t) {
        ...
        return self::result('tables', 'Tablas BD', 'Faltan: ' . implode(', ', $missing),
            self::SEVERITY_ERROR, 'Activa biodevas-common para crear las tablas compartidas.');
    }

    public static function check_cron(): array
    {
        $hooks = ['cst_hourly_event', 'cst_daily_event'];
        $missing = [];
        foreach ($hooks as $h) {
