...
$post = get_post($post_id);
        if (!$post || $post->post_type !== 'centro_turno') return;
        ...
        try {
            ...
            $horas_contabilizadas = (float) $wpdb->get_var($wpdb->prepare(
            ...
            if ($status === 'realizado' && $horas_contabilizadas <= 0) {
                ...
                if ($start_timestamp && $end_timestamp && $end_timestamp > $start_timestamp) {
                    ...
                    if ($hours > 0) {
                        ...
                        $log_entry_data = ['post_id' => $post_id, 'user_id' => $user_id, 'hours' => $hours];
