...
class Admin_Turnos_List extends \WP_List_Table
{
    ...
    protected function column_responsable($item): string
    {
        ...
        if (!$user_id) return '<span style="color:#94a3b8;">—</span>';
    ...
    public function prepare_items(): void
    {
        ...
        $meta_query = [];
        if ($filter_estado) {
            $meta_query[] = ['key' => '_estado', 'value' => $filter_estado];
        }
        if ($filter_asistencia) {
            $meta_query[] = ['key' => '_estado_real', 'value' => $filter_asistencia];
        }
        if ($filter_apoyo !== '') {
            $meta_query[] = ['key' => '_necesita_apoyo', 'value' => $filter_apoyo];
        }
        if (!empty($meta_query)) {
